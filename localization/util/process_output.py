...
def get_loc_results_from_raw_outputs(instance_id, raw_outputs, include_variable=False):
    ...
    for i, sample in enumerate(raw_outputs):
        ...
        logging.debug(f"Found files:             {found_files}")
        logging.debug(f"Found edit locations:        {found_edit_locs}")
        ...
        for entity in filtered_edit_entities:  
            if searcher.has_node(entity):  
                entity_data = searcher.get_node_data([entity], return_code_content=True)[0]  
                entity_with_code.append({  
                    'entity_id': entity,  
                    'type': entity_data['type'],  
                    'code': entity_data.get('code_content'),  
