...
def check_completion_status():
    """
    Use is_instance_completed function to count completed instances and calculate accuracy
    """
    # Set parameters
    base_dir = "/data/swebench/workspace/SWE-Search/tmp/SWE_Search_deepseek0324_verified75_REACT" #_REACT"
    max_iterations = 20
    instance_list_file = "/data/swebench/workspace/SWE-Search/verified75.txt"
    ...
    print(f"📋 Total instances to check: {len(instance_ids)}")
...
if __name__ == "__main__":
    ...
    if completed_count > 0:
        print(f"Accuracy (Completed): {resolved_count}/{completed_count} = {resolved_count/completed_count*100:.1f}%")
    else:
        print(f"  Accuracy (Completed): 0/0 = 0.0%") 